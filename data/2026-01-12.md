...
TL;DR: A nonparametric diffusion-detection test based on excursion counts and quadratic variation for continuous semimartingales; distinguishes diffusion from deterministic dynamics using a universal scaling law.


<details>
  <summary>Details</summary>
Motivation: Need to distinguish diffusion-like stochastic dynamics from deterministic signals using only a single time series, without relying on parametric models or subjective entropy-based criteria.

Method: Relies on excursion counts N_ε and the quadratic variation [X]_T of continuous semimartingales. For diffusions, N_ε scales with [X]_T and ε as per a universal law; compute empirical N_ε^{emp} and compare to theoretical N_ε^{theory}, form K(ε)=N_ε^{emp}/N_ε^{theory}, and use the log-log slope of K(ε) versus ε to classify as diffusion-like or not.

Result: Demonstrated on canonical stochastic systems, some periodic and chaotic maps, systems with additive white noise, and the stochastic Duffing system; shows robustness and model-free applicability.

Conclusion: Proposes a robust, data-driven diffusion test that is nonparametric and relies only on the small-scale universality of continuous semimartingales to distinguish stochastic diffusion from deterministic dynamics.
...
### [7] [MoEBlaze: Breaking the Memory Wall for Efficient MoE Training on Modern GPUs](https://arxiv.org/abs/2601.05296)
...
### [8] [Learn to Evolve: Self-supervised Neural JKO Operator for Wasserstein Gradient Flow](https://arxiv.org/abs/2601.05583)
*Xue Feng,Li Wang,Deanna Needell,Rongjie Lai*

Main category: cs.LG

TL;DR: 提出自监督 Learn-to-Evolve 方法，学习一个 JKO 解算器将输入密度直接映射到对应子问题的极小点，并可迭代应用以高效近似 Wasserstein 梯度流，无需对每条轨迹进行数值解。


<details>
  <summary>Details</summary>
Motivation: JKO 方案的数值实现通常成本高、效率低，限制了大规模或多初始条件下的应用，因此需要一个高效、稳健且具泛化能力的学习方法来近似梯度流的演化。

Method: 训练一个 JKO 运算符，将密度映射到相应 JKO 子问题最小点；通过 Learn-to-Evolve 交替进行轨迹生成和算子更新，训练过程中数据逐渐逼近真实 JKO 轨迹；自我生成的轨迹提供数据增强，显著提升泛化性。

Result: 在多种能量函数和初始条件下的数值实验表明，该方法具有较高的准确性、稳定性和鲁棒性，能够有效生成梯度流演化。

Conclusion: 无需对每条轨迹进行数值解即可实现自监督学习的 JKO 解算器，Learn-to-Evolve 通过数据自增广和交替训练提升泛化能力，适用于高效近似 Wasserstein 梯度流的计算。
...
### [9] [TIME: Temporally Intelligent Meta-reasoning Engine for Context Triggered Explicit Reasoning](https://arxiv.org/abs/2601.05300)
*Susmit Das*
...
### [10] [Ontology Neural Networks for Topologically Conditioned Constraint Satisfaction](https://arxiv.org/abs/2601.05304)
*Jaehong Oh*
...
### [11] [When the Server Steps In: Calibrated Updates for Fair Federated Learning](https://arxiv.org/abs/2601.05352)
...
### [12] [Poisson Hyperplane Processes with Rectified Linear Units](https://arxiv.org/abs/2601.05586)
...
### [13] [GlyRAG: Context-Aware Retrieval-Augmented Framework for Blood Glucose Forecasting](https://arxiv.org/abs/2601.05353)
*Shovito Barua Soumma,Hassan Ghasemzadeh*

Main category: cs.LG

TL;DR: GlyRAG 使用 LLM 作为情境化分析器，并通过检索增强的多模态 transformer，在CGM数据上进行血糖预测，且在两组1型糖尿病患者数据集上显著提升 RMSE 与 dysglycemia 预测效果，无需额外传感器。


<details>
  <summary>Details</summary>
Motivation: 解决当前 CGM 序列化处理忽视上下文信息或依赖难以扩展的传感器的问题，探索基于LLM的情境化理解和病例类比检索来提升长期预测准确性和临床可用性。

Method: 将 CGM 序列与 LLM 生成的临床摘要嵌入，通过语言模型嵌入与补丁级 glucose 表征的多模态 transformer 融合，使用跨翻译损失对文本与生理嵌入对齐。建立一个检索模块，在学习的嵌入空间中查找相似历史事件并通过 cross-attention 将这些案列类比融入预测过程。

Result: 在两个T1D队列上，GlyRAG 相较于最先进方法，RMSE 降低最高达到39%，在基线基础上再降1.7%；临床评估显示预测进入安全区比例85%，预测低血糖/高血糖事件的能力提升51%（在两个队列中）。

Conclusion: LLM 导向的情景化理解与检索式情节对CGM序列的解析能提高长期血糖预测的准确性和临床可靠性，无需额外传感器，支持未来面向决策支持的 diabetes management。
...
### [14] [Auditing Fairness under Model Updates: Fundamental Complexity and Property-Preserving Updates](https://arxiv.org/abs/2601.05909)
...
### [15] [The Kernel Manifold: A Geometric Approach to Gaussian Process Model Selection](https://arxiv.org/abs/2601.05371)
...
### [16] [Imitation Learning for Combinatorial Optimisation under Uncertainty](https://arxiv.org/abs/2601.05383)
*Prakash Gawas,Antoine Legrain,Louis-Martin Rousseau*

Main category: cs.LG

TL;DR: 提出一个对抗演示的三维专家分类法，扩展 DAgger 以支持多专家查询与互动学习，并在动态分诊分配问题上验证，结果显示随机/不确定性更高的专家优于确定性或全信息专家，互动学习在较少演示下提升解质量，聚合的确定性专家在计算受限情形下是有效替代。


<details>
  <summary>Details</summary>
Motivation: 在将 imitation learning 应用于大规模组合优化的序列决策问题时，缺乏统一的专家建模框架及其对学习性能影响的理解。

Method: 提出一个通用的专家分类框架，基于对不确定性处理、任务最优性以及学习交互方式的三维划分，并在此基础上给出一个广义的 Dataset Aggregation (DAgger) 算法，支持多专家查询、专家聚合和灵活的交互策略。

Result: 在一个具有随机到达和容量约束的动态医生-患者分配问题上进行实验，比较不同专家类型和交互策略的学习结果：来自随机/随机化的专家往往优于确定性或全信息专家；互动学习在较少演示下提升解的质量；聚合的确定性专家在难以进行随机优化时提供有效替代。

Conclusion: 该框架揭示专家选择对 IL 性能的显著影响，随机/高不确定性专家具有优势；互动式学习与专家聚合能提高样本效率；在计算受限情形下，聚合的确定性专家可作为低成本替代方案。
...
### [17] [DynaSTy: A Framework for SpatioTemporal Node Attribute Prediction in Dynamic Graphs](https://arxiv.org/abs/2601.05391)
*Namrata Banerji,Tanya Berger-Wolf*

Main category: cs.LG

TL;DR: Dynamic edge-biased spatiotemporal transformer for multistep node attribute forecasting on evolving graphs; handles dynamic adjacency; uses attention bias with adjacency; pretraining + scheduled sampling; horizon-weighted loss; outperforms baselines on RMSE/MAE.


<details>
  <summary>Details</summary>
Motivation: Forecasting node-level attributes on dynamic graphs where the adjacency structure evolves, enabling multi-system settings (brain networks, financial systems, social systems) and addressing compounding errors over long horizons.

Method: An end-to-end model that ingests a multivariate time series of node attributes and a time series of adjacency matrices. At each step, a transformer-based encoder uses the given adjacency as an adaptable attention bias to focus on relevant neighbors as the graph changes. Training includes a masked node-time pretraining objective, scheduled sampling, and a horizon-weighted loss to mitigate error accumulation.

Result: Empirical results show the proposed method consistently outperforms strong baselines in RMSE and MAE across evaluated tasks.

Conclusion: Incorporating dynamic adjacency as a contextual attention bias enables accurate multistep forecasting on evolving graphs and supports forecasting across diverse systems.
...
### [27] [DeMa: Dual-Path Delay-Aware Mamba for Efficient Multivariate Time Series Analysis](https://arxiv.org/abs/2601.05527)
...
### [28] [Scalable Heterogeneous Graph Learning via Heterogeneous-aware Orthogonal Prototype Experts](https://arxiv.org/abs/2601.05537)
...
### [29] [Buffered AUC maximization for scoring systems via mixed-integer optimization](https://arxiv.org/abs/2601.05544)
...
### [30] [PaCoRe: Learning to Scale Test-Time Compute with Parallel Coordinated Reasoning](https://arxiv.org/abs/2601.05593)
...
### [31] [Good Allocations from Bad Estimates](https://arxiv.org/abs/2601.05597)
...
### [32] [Orchestrating Tokens and Sequences: Dynamic Hybrid Policy Optimization for RLVR](https://arxiv.org/abs/2601.05607)
...
### [33] [PiXTime: A Model for Federated Time Series Forecasting with Heterogeneous Data Structures Across Nodes](https://arxiv.org/abs/2601.05613)
...
### [34] [Continual Learning of Achieving Forgetting-free and Positive Knowledge Transfer](https://arxiv.org/abs/2601.05623)
...
### [35] [Transformer Is Inherently a Causal Learner](https://arxiv.org/abs/2601.05647)
...
### [36] [From Global to Local: Cluster-Aware Learning for Wi-Fi Fingerprinting Indoor Localisation](https://arxiv.org/abs/2601.05650)
...
### [37] [Do Sparse Autoencoders Identify Reasoning Features in Language Models?](https://arxiv.org/abs/2601.05679)
...
### [40] [mHC-lite: You Don't Need 20 Sinkhorn-Knopp Iterations](https://arxiv.org/abs/2601.05732)
...
### [41] [Variational Autoencoders for P-wave Detection on Strong Motion Earthquake Spectrograms](https://arxiv.org/abs/2601.05759)
...
### [42] [Weights to Code: Extracting Interpretable Algorithms from the Discrete Transformer](https://arxiv.org/abs/2601.05770)
...
### [43] [Tensor-DTI: Enhancing Biomolecular Interaction Prediction with Contrastive Embedding Learning](https://arxiv.org/abs/2601.05792)
...
### [44] [Fusion Matters: Length-Aware Analysis of Positional-Encoding Fusion in Transformers](https://arxiv.org/abs/2601.05807)
*Mohamed Amine Hallam,Kuo-Kun Tseng*

Main category: cs.LG

TL;DR: 在长序列中，位置编码的融合方式对性能有显著影响；短序列影响甚少；可学习的融合和轻量卷积门控在长文跨位置编码家族具泛化性。


<details>
  <summary>Details</summary>
Motivation: 探究位置编码信息如何与 token embeddings 融合，以及这种融合机制是否影响 Transformer 在不同序列长度上的表现，补充仅设计新编码的研究倾向。

Method: 在相同架构、数据划分和随机种子下，对三种融合策略（元素级相加、拼接再投影、标量门控）在 AG News、IMDB、ArXiv 上进行对比；进行配对种子分析与跨数据集比较；在 ArXiv 上测试可学习融合对多种位置编码族的泛化；并在长文上尝试轻量卷积门控以引入局部感知。

Result: 融合策略对短文本几乎无影响，但在长文上带来稳定的性能提升；可学习融合在多位置编码族上具有泛化性；配对种子分析与跨数据集对比表明提升是结构性的；轻量卷积门控在长文本上也展现潜在收益。

Conclusion: 位置编码的融合是长序列 Transformer 的一个重要设计选择，应该作为明确的建模决策来优化，而不是简单沿用固定默认策略。
...
### [45] [Learning Reconstructive Embeddings in Reproducing Kernel Hilbert Spaces via the Representer Theorem](https://arxiv.org/abs/2601.05811)
...
### [46] [Detecting Autism Spectrum Disorder with Deep Eye Movement Features](https://arxiv.org/abs/2601.05812)
...
TL;DR: A learning framework that treats asymptotic integration constants as scale-dependent functions, constrained by the governing differential equation to produce a globally valid solution without explicit boundary matching.


<details>
  <summary>Details</summary>
Motivation: Address limitations of traditional multi-region (patch) methods in divided-domain asymptotics by learning smooth, globally valid coefficient functions that interpolate between regimes.

Method: Replace fixed asymptotic constants with functions of scale; enforce the original differential equation across the domain (likely via a neural network or similar surrogate) to train these coefficient functions to satisfy the DE and match asymptotic behavior.

Result: The approach reproduces global solutions accurately and outperforms conventional matching in problems from chemical kinetics and cosmology.

Conclusion: Offers a robust, general framework for unifying asymptotic regions without ad-hoc boundary matching, with potential applicability across physics and engineering.
...
### [50] [Distilling Lightweight Domain Experts from Large ML Models by Identifying Relevant Subspaces](https://arxiv.org/abs/2601.05913)
...
### [51] [Prophet as a Repro ducible Forecasting Framework: A Methodological Guide for Business and Financial Analytics](https://arxiv.org/abs/2601.05929)
...
### [52] [On the Robustness of Age for Learning-Based Wireless Scheduling in Unknown Environments](https://arxiv.org/abs/2601.05956)
...
### [53] [Community-Based Model Sharing and Generalisation: Anomaly Detection in IoT Temperature Sensor Networks](https://arxiv.org/abs/2601.05984)
...
### [54] [LookAroundNet: Extending Temporal Context with Transformers for Clinically Viable EEG Seizure Detection](https://arxiv.org/abs/2601.06016)
