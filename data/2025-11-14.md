...
### [1] [Masked Mineral Modeling: Continent-Scale Mineral Prospecting via Geospatial Infilling](https://arxiv.org/abs/2511.09722)
*Sujay Nair,Evan Coleman,Sherrie Wang,Elsa Olivetti*

Main category: stat.ML

TL;DR: 基于掩码-填充的生成模型用于推断地下矿产位置，通过对地理空间资源分布进行掩码和填充来学习。利用美国大陆数据，最佳模型在1×1英里网格上达到 Dice 约0.31、召回约0.22。引入辅助数据（地球物理特征、土壤调查等）可提升推断性能，并使在缺矿区域也能进行评估。


<details>
  <summary>Details</summary>
Motivation: 矿产对先进能源技术至关重要，但地下矿床的勘探成本高、难度大。受最近生成模型进展启发，提出通过掩码-填充的地理空间推断来定位矿产的学习方法，利用更丰富的辅助数据来提升对矿产分布的推断能力。

Method: 将地理空间资源分布进行掩码处理并进行填充/重建，训练生成模型来预测未知区域的矿产存在概率。以美国除夏威夷以外的连桶区数据为训练与测试集，在1×1英里网格分辨率上评估。模型可易于接入额外输入层（如地球物理数据、广泛的土壤调查数据），以提升推断性能并在无矿记录区域进行模型评估。

Result: 最佳模型在测试数据上获得 Dice 系数约0.31±0.01、召回率约0.22±0.02（1×1英里网格）。引入辅助特征（地球物理层、全国土壤调查等）显著提升推断性能，并使模型能够在没有矿产记录的区域进行评估。

Conclusion: 基于掩码-填充的生成方法可用于矿产探测，并且通过引入辅助数据可提升预测性能与区域覆盖，尤其有助于在矿产数据稀缺区域实现推断与评估。
...
### [2] [Generalized infinite dimensional Alpha-Procrustes based geometries](https://arxiv.org/abs/2511.09801)
*Salvish Goomanee,Andi Han,Pratik Jawanpuria,Bamdev Mishra*

Main category: stat.ML

TL;DR: 将 Alpha-Procrustes SPD 距离框架推广到广义的 Bures-Wasserstein、对数欧氏、以及 Wasserstein 距离在无限维情形下的几何距离；通过单位化的希尔伯特-施瓦茨算子和扩展的 Mahalanobis 范数构造，结合可学习的正则化参数，提升高维数据下的鲁棒性比较能力；初步实验显示在跨维度/尺度的数据集上具有更好表现。


<details>
  <summary>Details</summary>
Motivation: 建立一个统一且可扩展的 SPD 流形下的广义距离族，以结合 GBW、对数欧氏和 Wasserstein 等距离的结构，同时在无限维设定中保持数值稳定与鲁棒性，促进机器学习、统计推断以及函数数据分析中的几何方法。

Method: 提出基于单位化希尔伯特-施瓦茨算子和扩展 Mahalanobis 范数的形式化框架，能够构造 GBW、Log-Hilbert-Schmidt 等无限维通用距离；在 Alpha-Procrustes 框架内整合可学习的正则化参数以增强高维比较的几何稳定性。并给出理论与计算基础以实现这些广义距离。

Result: 通过再现文献基准的初步实验，证实广义距离在跨数据维度与尺度的比较中具有改善的性能表现。

Conclusion: 建立了理论与计算基础，为机器学习、统计推断和函数数据分析中的鲁棒几何方法提供新的通用距离工具及可学习的正则化机制。
...
### [3] [Theory and computation for structured variational inference](https://arxiv.org/abs/2511.09897)
*Shunan Sheng,Bohan Wu,Bennett Zhu,Sinho Chewi,Aram-Alexandre Pooladian*

Main category: stat.ML

TL;DR: 本论文将星型结构变分推断理论化：证明存在性、唯一性与自洽性，给出从均场扩展到星型的逼近误差界，提出基于最优传输思想的梯度算法并具备理论保证；并探讨高斯测度与分层贝叶斯模型中的应用（包括广义线性模型、带单维去偏的 Spike-and-Slab），以及星-可分传输映射的稳定性结果。


<details>
  <summary>Details</summary>
Motivation: 结构化变分推断在实际应用中越来越受关注，但大多数结果停留在均场近似。星型结构（根变量影响所有其他变量）是一个自然且有用的设定。本工作旨在建立星型结构变分近似的存在、唯一性与自洽性，并提供误差界与可实现的优化算法，以提升对后验分布的近似质量，并拓展至常见统计模型。

Method: 从理论角度证明星型结构下的变分近似存在性、唯一性与自洽性；给出后验近似的定量误差界，并将均场的结果扩展到星型结构；提出一个基于梯度的算法，融合最优传输理论的思想，具有渐近收敛性与稳定性保证；对高斯测度、分层贝叶斯模型、带位置族先验的 GLM、以及带单维去偏的 Spike-and-Slab 进行具体推导；同时给出星-可分传输映射的稳定性分析作为独立结果。

Result: 得到星型结构变分近似的存在、唯一性与自洽性定理；建立了从均场到星型的定量误差界；提出并证明具备收敛性和稳定性保证的梯度优化算法；给出对高斯、分层贝叶斯以及相关模型的具体应用框架和推导；并获得星-可分传输映射的稳定性结果。

Conclusion: 将变分推断理论从均场拓展到星型结构，提供可操作且带有理论保证的近似与算法，能够有效应用于广义线性模型、Spike-and-Slab 等常见统计模型，并揭示星-可分传输映射的稳定性可能具有独立的理论价值。
...
### [4] [Operator Models for Continuous-Time Offline Reinforcement Learning](https://arxiv.org/abs/2511.10383)
*Nicolas Hoischen,Petar Bevanda,Max Beier,Stefan Sosnowski,Boris Houska,Sandra Hirche*

Main category: stat.ML

TL;DR: 提出一个基于算子理论的离线强化学习框架，使用连续时间受控扩散过程的生成器在再生核希尔空间中学习世界模型，通过与哈密尔顿-雅可比-贝尔曼方程(HJB)的联系，给出全局收敛性与有限样本误差界的理论与数值分析。


<details>
  <summary>Details</summary>
Motivation: 解决离线数据学习策略时对近似误差的统计理解不足的问题；将强化学习与HJB及算子理论结合，旨在在连续时间设置中提供理论保证。

Method: 将世界模型表示为受控扩散过程的无穷小生成器，并在再生核希尔空间中学习；基于简单的动态规划递推提出以算子为核心的离线RL算法；给出收敛性和误差分析。

Result: 证明值函数的全局收敛性；推导出与系统光滑性、稳定性等属性相关的有限样本误差界；数值实验显示算子基方法在离线连续时间最优控制中的潜力。

Conclusion: 算子基方法在离线离线时间最优控制框架下的连续时间强化学习问题中具有潜在价值，未来研究可进一步融合统计学习以提升鲁棒性和样本效率。
...
### [5] [Continuum Dropout for Neural Differential Equations](https://arxiv.org/abs/2511.10446)
*Jonghun Lee,YongKyung Oh,Sungil Kim,Dong-Young Lim*

Main category: stat.ML

TL;DR: 引入 Continuum Dropout——一种适用于神经微分方程的连续时间正则化方法，基于交替更新过程理论，通过将 dropout 的开关设定为在连续时间中交替处于激活与暂停，从而抑制过拟合并提供不确定性量化；在时间序列和图像分类任务中表现优于现有正则化方法，并给出更好校准的概率估计。


<details>
  <summary>Details</summary>
Motivation: NDEs 在处理不规则观测、缺失数据和噪声方面具有优势，但在采用 dropout 进行正则化时存在本质挑战，易导致过拟合；需要一种可在连续时间演化中使用、且能提供不确定性量化的正则化策略。

Method: 将 dropout 的开关建模为在连续时间中交替的活跃（演化）与非活跃（暂停）状态的随机过程，基于交替更新过程理论构建 Continuum Dropout；在测试阶段通过蒙特卡罗采样实现预测不确定性量化。

Result: 大量实验显示 Continuum Dropout 在多种时间序列和图像分类任务上优于现有正则化方法，且得到更良好校准的概率估计。

Conclusion: Continuum Dropout 提供一个普适且有理论支撑的 NDE 正则化框架，促进不确定性建模的可行性和可信度提升。
...
### [6] [Incremental Generation is Necessity and Sufficient for Universality in Flow-Based Modelling](https://arxiv.org/abs/2511.09902)
*Hossein Rouhvarzi,Anastasis Kratsios*

Main category: cs.LG

TL;DR: Incremental flow-based generation is necessary and sufficient for universal generation on the largest natural class of self-maps of [0,1]^d (orientation-preserving homeomorphisms). Single-step autonomous flows are not universal, but a finite composition of such flows achieves universal approximation with rate O(n^{-1/d}); under smoothness, rates can be dimension-free. Lifting to a higher dimension yields universal results for continuous functions and for probability measures (via pushforwards with vanishing 1-Wasserstein error).


<details>
  <summary>Details</summary>
Motivation: Provide a rigorous approximation-theoretic foundation for incremental flow-based denoising models, and establish uniform approximation guarantees (samplewise and distributional) for a broad class of self-maps compatible with denoising pipelines.

Method: Develop a topological-dynamical framework. Prove an impossibility theorem showing single-step autonomous flows are meagre in the space of orientation-preserving homeomorphisms. Then construct approximations of every such homeomorphism by a finite composition of autonomous flows, leveraging algebraic properties. Quantify the rate O(n^{-1/d}) and show dimension-free rates under extra smoothness. Use a domain lifting trick to extend results to continuous functions and probability measures (as pushforwards of empirical measures) with vanishing 1-Wasserstein error.

Result: Impossibility: single-step autonomous flows cannot universally approximate orientation-preserving homeomorphisms. Positive result: any orientation-preserving Lipschitz homeomorphism on [0,1]^d can be approximated to rate O(n^{-1/d}) by a composition of at most K_d autonomous flows. With smootherness, rates can be dimension-free and K_d uniform over the class. Additional lifting yields universal approximation for continuous functions and for probability measures under 1-Wasserstein metric.

Conclusion: Incremental generation is the right and sometimes necessary paradigm for universal flow-based generation on the natural class considered; a finite, dimension-dependent (and potentially dimension-free under smoothness) composition of autonomous flows suffices. The approach yields broad approximation guarantees for functions and measures via liftings, bridging samplewise and distributional convergence.
...
TL;DR: A method for automated ICD coding uses a Directed Bipartite Graph Encoder distinguishing common vs rare codes, with one-way edges from common to rare. It injects a Co-occurrence Encoding bias into attention, enriched by LLM-generated code descriptions. Demonstrates state-of-the-art results, especially in Macro-F1 for long-tail codes.


<details>
  <summary>Details</summary>
Motivation: Address the long-tail, large label space challenge in automated ICD coding by leveraging fine-grained co-occurrence relationships between codes and providing richer initial embeddings through LLM-derived descriptions.

Method: Construct a Directed Bipartite Graph Encoder with disjoint sets: common codes and rare codes. Edges go from common to rare; bias is defined by the conditional probability of a common code given the presence of a rare code. This bias is injected into the encoder's attention (Co-occurrence Encoding) to enrich rare code representations via latent comorbidity information. Additionally, use an LLM to generate comprehensive code descriptions to augment initial embeddings, providing external knowledge for co-occurrence relationships.

Result: Experiments on three ICD coding benchmarks show state-of-the-art performance, with particularly notable improvements in Macro-F1, the key metric for long-tail classification.

Conclusion: A co-occurrence aware, directionally biased graph encoder, aided by LLM-derived code descriptions, can significantly improve performance for rare codes in large-label ICD coding tasks, achieving state-of-the-art results and better macro performance.
...
### [8] [Let the Experts Speak: Improving Survival Prediction & Calibration via Mixture-of-Experts Heads](https://arxiv.org/abs/2511.09567)
*Todd Morrill,Aahlad Puli,Murad Megjhani,Soojin Park,Richard Zemel*

Main category: cs.LG

TL;DR: 提出多种离散时间深度MoE（专家-模型）用于生存分析，探索在保持预测准确性与校准的同时发现患者分组结构。


<details>
  <summary>Details</summary>
Motivation: 现有深度MoE在分组带来可解释性之余，往往以牺牲校准和预测准确性为代价。希望在发现分组结构的同时提升预测性能和校准度。

Method: 提出若干离散时间的深度MoE架构用于生存分析，其中有一种架构同时实现聚类、校准和预测准确性。关键在于专家的表达能力：更具表达力的专家能够对单个患者进行个性化预测，优于基于固定原型分组的专家。

Result: 实验表明，专家表达能力越强，模型对单个患者的预测越精确，且部分模型实现了聚类、校准与预测准确性三者兼具。

Conclusion: 提升MoE的专家表达能力是实现患者分组发现与改善校准及预测的关键；在离散时间生存分析中，存在能够同时满足聚类、校准与准确性的MoE架构。
...
### [9] [Filtering Jump Markov Systems with Partially Known Dynamics: A Model-Based Deep Learning Approach](https://arxiv.org/abs/2511.09569)
*George Stamatelis,George C. Alexandropoulos*

Main category: cs.LG

TL;DR: 提出 Jump Markov Filtering Network (JMFNet)，一个用于具有未知噪声统计与模式转移动态的跳跃马尔可夫系统的实时状态估计的模型驱动深度学习框架。通过两个RNN的混合架构实现模式预测与基于模式扩展的 KalmanNet 过滤，并通过交替最小二乘法进行联合训练，在潜在模式无监督的情况下实现互相适应。


<details>
  <summary>Details</summary>
Motivation: 在跳跃马尔可夫系统中进行实时状态估计，面临噪声统计与模式转移动力学未知、传统滤波器（如IMM、粒子滤波）对非平稳与高噪声场景的不足，以及模型驱动与模型无关深度学习之间的权衡。

Method: 提出两支RNN组成的混合架构：一支用于模式预测，另一支基于模式扩展的 KalmanNet 进行滤波；通过交替最小二乘法端到端联合训练，使潜在模式无需监督即可实现互相自适应。对线性与非线性系统（目标跟踪、摆角跟踪、洛伦兹吸引子、真实数据集）进行广泛实验。

Result: 在非平稳与高噪声条件下，显著优于经典模型基滤波器（如互动多模型、粒子滤波）及模型无关的深度学习基线；对 KalmanNet 的提升在复杂系统/较长轨迹中更为明显；结论为 JMFNet 在一致性、鲁棒性方面表现良好，对初始条件、超参数及模型知识误差的敏感性低。

Conclusion: JMFNet 为跳跃马尔可夫系统的实时状态估计提供了一个高效、可靠的联合模型驱动解决方案，能在未知噪声与模式转移场景下实现优于传统滤波与部分增强的鲁棒性，适用于复杂动力学与高噪声环境。
...
### [10] [Group Averaging for Physics Applications: Accuracy Improvements at Zero Training Cost](https://arxiv.org/abs/2511.09573)
*Valentino F. Foit,David W. Hogg,Soledad Villar*

Main category: cs.LG

TL;DR: 对某些微分方程相关的机器学习模型，通过在评估时对小规模对称群进行平均，可以以很低成本强制实现精确对称性，从而普遍降低评估误差并改善预测的可视性。


<details>
  <summary>Details</summary>
Motivation: 解决在自然科学中的对称性问题，尽管有方法可实现对称性，但往往训练困难、需要学习对称性，或实现复杂。群平均提供一种评估时的无结构方法来提升模型性能。

Method: 在已有的微分方程相关的标准ML模型上，在评估时对一个小的对称群进行群平均，将模型输出进行求平均（或对模型进行平均）以实现精确的对称性，成本与对称群大小成正比。

Result: 平均评估损失下降；VRMSE提升最高可达37%；预测在视觉上更平滑、符合连续动力行为。

Conclusion: 在常见情境下，强制对称性并无负面影响，应该将群平均视为一种廉价且简单的提升手段，鼓励ML4PS社区采用。
...
### [11] [Boosted GFlowNets: Improving Exploration via Sequential Learning](https://arxiv.org/abs/2511.09677)
...
### [12] [HeatGen: A Guided Diffusion Framework for Multiphysics Heat Sink Design Optimization](https://arxiv.org/abs/2511.09578)
...
### [13] [Scaling Environments for LLM Agents in the Era of Learning from Interaction: A Survey](https://arxiv.org/abs/2511.09586)
...
### [14] [A Novel Data-Dependent Learning Paradigm for Large Hypothesis Classes](https://arxiv.org/abs/2511.09996)
*Alireza F. Pour,Shai Ben-David*

Main category: cs.LG

TL;DR: 提出一种新的学习范式，在候选模型数量过大时，避免对齐过多的先验假设，通过更强的经验数据利用来提升泛化，而不必依赖统一收敛或严格正则化的约束。


<details>
  <summary>Details</summary>
Motivation: 在具有庞大模型集合时，传统的经验风险最小化或正则化学习可能难以获得良好泛化；需要一种更充分利用经验数据且对先验假设依赖更少的方法。

Method: 开发一种强化经验数据利用的新学习范式，允许在无需事先给出真实参数的情况下，利用点间相似性、域的聚类、标签规则的Lipschitz性以及对比学习假设等常见假设来提升泛化，同时分析其泛化能力。

Result: 理论上给出在多种常见学习假设下的泛化能力分析，以及在近邻相似性、聚类结构、Lipschitz性和对比学习假设等场景的优点；不需要提前知道参数真值。

Conclusion: 该范式为在不依赖精确参数先验的情况下，利用这些假设提升泛化能力，提供一种可用于大量模型集合的替代或补充方法，减小对先验假设的依赖。
...
### [16] [Making Every Head Count: Sparse Attention Without the Speed-Performance Trade-off](https://arxiv.org/abs/2511.09596)
...
### [18] [Optimistic Reinforcement Learning with Quantile Objectives](https://arxiv.org/abs/2511.09652)
...
### [19] [Generalization Can Emerge in Tabular Foundation Models From a Single Table](https://arxiv.org/abs/2511.09665)
...
### [20] [GEM+: Scalable State-of-the-Art Private Synthetic Data with Generator Networks](https://arxiv.org/abs/2511.09672)
...
### [21] [SEBA: Sample-Efficient Black-Box Attacks on Visual Reinforcement Learning](https://arxiv.org/abs/2511.09681)
...
### [22] [ConstrainedSQL: Training LLMs for Text2SQL via Constrained Reinforcement Learning](https://arxiv.org/abs/2511.09693)
...
TL;DR: 提出一种约束强化学习框架用于 Text2SQL，使用可解释的奖励与约束信号并在训练中动态平衡，给出理论保证并在典型数据集上优于现有的 RL-LMs。


<details>
  <summary>Details</summary>
Motivation: 现有基于强化学习的 Text2SQL 学习严重依赖奖励函数设计，易出现奖励劫持（reward hacking），需要更自然、可解释的奖励与约束信号，并在训练过程中动态权衡它们。

Method: 构建一个带约束的强化学习框架，整合奖励信号与约束信号，并在训练中对两者的权重进行动态调整；给出理论保障（如收敛性/最优性等）并在 Text2SQL 数据集上进行数值实验以验证效果。

Result: 给出了理论保证，实验结果在著名 Text2SQL 数据集上优于当前最先进的 RL 训练 LLMs，体现了框架的有效性。

Conclusion: 通过引入可解释的约束型 RL，能够缓解奖励设计带来的问题并提升 Text2SQL 的推理能力和生成质量。
...
### [23] [Efficient Hyperdimensional Computing with Modular Composite Representations](https://arxiv.org/abs/2511.09708)
...
### [24] [Generalizing PDE Emulation with Equation-Aware Neural Operators](https://arxiv.org/abs/2511.09729)
...
### [25] [FlowCast: Advancing Precipitation Nowcasting with Conditional Flow Matching](https://arxiv.org/abs/2511.09731)
*Bernardo Perrone Ribeiro,Jana Faganeli Pucer*

Main category: cs.LG

TL;DR: FlowCast首次将条件流匹配（CFM）应用于降水今世casting，通过直接学习噪声到数据的映射，以比扩散更高效的采样实现高保真预测，达到新的最先进性能。


<details>
  <summary>Details</summary>
Motivation: 解决大气动力学的不确定性和高维时空数据的高效建模需求，提升降水今世casting的精度与实时性。

Method: 在降水今世casting 中采用CFM，学习直接的噪声到数据映射，减少采样步骤，与传统扩散模型在同一架构上进行对比。

Result: CFM在同一架构下比扩散目标更准确且更加高效，获得新颖的状态-of-the-art预测性能；显示CFM作为高维时空预测的有效替代。

Conclusion: CFM为高维时空降水今世casting提供了一个高效且准确的替代方法，显著降低采样成本并提升预测质量。
...
### [26] [Out-of-Distribution Generalization with a SPARC: Racing 100 Unseen Vehicles with a Single Policy](https://arxiv.org/abs/2511.09737)
...
### [27] [TawPipe: Topology-Aware Weight Pipeline Parallelism for Accelerating Long-Context Large Models Training](https://arxiv.org/abs/2511.09741)
*Houming Wu,Ling Chen*

Main category: cs.LG

TL;DR: TawPipe通过拓扑感知的权重流水并行，利用分层带宽，降低跨节点通信并提升GPU群组内吞吐和可扩展性；在多达24个GPU、LLaMA风格模型的实验中，优于现有基线。


<details>
  <summary>Details</summary>
Motivation: 训练大语言模型受限于设备显存和跨设备通信成本。流水并行能缓解显存压力，但激活传输随序列长度线性增长，效率受限。现有权重传输方法如 WeiPipe 遭遇冗余的点对点传输与节点内带宽未被充分利用的问题，亟需在拓扑层面优化通信以提升整体吞吐。

Method: 将设备按拓扑分组以优化节点内的聚合通信与节点间的点对点通信；为每个设备分配固定的模型权重和梯度分片，避免冗余传输；在计算阶段重叠通信以隐藏延迟；与全局集体操作的 FSDP 不同，尽量将大部分通信限定在节点边界内以减少跨节点流量。

Result: 在多达24个GPU的实验中，使用 LLaMA 风格模型时，TawPipe 相较于最先进的基线展现出更高的吞吐量和更好的可扩展性。

Conclusion: 通过把通信限制在节点内部、实现通信与计算的重叠、并避免冗余传输，TawPipe 能显著提升大规模语言模型训练的效率，对具有分层带宽的集群具备较强适用性。
...
### [28] [History Rhymes: Macro-Contextual Retrieval for Robust Financial Forecasting](https://arxiv.org/abs/2511.09754)
*Sarthak Khanna,Armin Berger,Muskaan Chopra,Rafet Sifa*

Main category: cs.LG

TL;DR: 提出一种宏观-contextual检索的预测框架，将宏观指标和财经新闻情绪嵌入同一相似性空间，通过检索历史上相似的宏观情境来支撑每次预测，无需重新训练，提升分布外（OOD）鲁棒性并实现可解释性。对比基线在OOd场景下表现优于静态数值、文本仅和简单多模态方法，且在AAPL和XOM的实际交易中取得正向结果。数据和代码公开。


<details>
  <summary>Details</summary>
Motivation: 金融市场高度非平稳，结构性突破和宏观经济情景切换常导致预测模型在分布外失效。现有的多模态融合方法对情景变化缺乏自适应能力。本工作提出基于宏观情景的检索式预测框架，通过在历史相似宏观情境中检索先例来支撑当前预测，提升鲁棒性与可解释性。

Method: 将宏观指标（如CPI、失业率、收益率曲线、GDP增速）与财经新闻情绪嵌入到一个共享的相似性空间；在推理阶段通过相似度检索找到历史上与当前宏观情景相近的时段作为“先例”，并以此支撑预测，整个过程无需重新训练。训练基于2007-2023年的S&P 500 数据，评估在OOD情境下对AAPL和XOM的预测与交易表现，并给出可解释的证据链（检索到的先例时段及其宏观上下文）。该框架实现了“历史可能不重复，但往往押韵”的理念。

Result: 在OOD条件下显著缩小预测分布差距的性能差距；宏观条件下的检索成为唯一实现正向样本外交易收益的方案（AAPL PF=1.18, Sharpe=0.95；XOM PF=1.16, Sharpe=0.61）。静态数值、文本单模态及简单多模态基线在情景切换下表现崩溃。检索到的邻居可形成可解释的证据链，对应于通胀期、收益率曲线反转等宏观情景，提升因果可解释性与透明度。数据、模型与代码公开。

Conclusion: 宏观意识的检索式预测在面对分布变更时提供鲁棒且可解释的前瞻性预测能力，验证了“金融历史可能不重复，但常会押韵”的直觉。该方法通过将宏观信息与文本情绪嵌入统一空间并进行先例检索，成功实现对OOD的稳健预测并具备可追溯的证据链。
...
### [29] [Is nasty noise actually harder than malicious noise?](https://arxiv.org/abs/2511.09763)
*Guy Blanc,Yizhi Huang,Tal Malkin,Rocco A. Servedio*

Main category: cs.LG

TL;DR: 对抗性噪声学习的分布设定显著影响可学习性：在分布无关设定下，恶意噪声与 nasty 噪声的学习能力等价；在固定分布设定下存在任意大的学习能力分离；引入 ICE 算法后，两噪声在近似等价的区间内可转化且系数界限为 2，且该界限在密码学假设下最优。


<details>
  <summary>Details</summary>
Motivation: 揭示对抗性噪声对高效学习边界的依赖性，比较不同设定下恶意和 nasty 噪声对布尔函数学习能力的影响，并提供一种在固定分布下更稳健的学习范式。

Method: 对比分析两类噪声模型在分布独立和固定分布两种设定下的可学习性，给出等价与分离结论；提出 ICE（ignore-contradictory examples）算法族，证明在 ICE 框架下恶意与 nasty 噪声的关系以及最优性下界（系数为 2）。

Result: 分布独立：若在 η 速率的恶意噪声下可学习，则同样在 η 速率下可学习 nasty 噪声；固定分布：存在可任意放大的 η_malicious/η_nasty 比值；ICE：高效 ICE 学习器若在 η 时对恶意噪声有效，可转化为在 η/2 时对 nasty 噪声有效，且该 2 倍界限在密码学假设下不可被改进。

Conclusion: 对抗性噪声学习的可学习性高度依赖分布设定；ICE 提供在固定分布设定下的近似等价与鲁棒性分析，并给出最优界限。
...
### [30] [Hail to the Thief: Exploring Attacks and Defenses in Decentralised GRPO](https://arxiv.org/abs/2511.09780)
*Nikolay Blagoev,Oğuzhan Ersoy,Lydia Yiyu Chen*

Main category: cs.LG

TL;DR: A novel adversarial poisoning attack on decentralised Group Relative Policy Optimization (GRPO) for LLMs; the attack can easily poison benign nodes with up to 100% success within ~50 iterations, and defenses can deter the attack with up to 100% stop rate.


<details>
  <summary>Details</summary>
Motivation: GRPO enables low-communication, decentralised post-training of LLMs by answering prompts across multiple nodes and exchanging them as strings. This architecture is vulnerable to poisoning attacks because malicious tokens can be injected into benign models during post-training, compromising the learning process.

Method: Demonstrate adversarial attacks by injecting arbitrary malicious tokens in benign models in both out-of-context and in-context settings using math and coding tasks. Measure attack success across iterations. Propose two defense schemes depending on whether all users train the same model or different models, and evaluate their effectiveness by stop rates.

Result: Adversarial attacks can poison benign nodes and contaminate local LLM post-training, achieving attack success rates up to 100% in as few as 50 iterations. Defenses can achieve stop rates up to 100%, rendering the attack ineffective.

Conclusion: Decentralised GRPO is vulnerable to poisoning attacks via token-level tampering, but appropriate defense strategies—depending on model homogeneity across users—can fully mitigate the threat and prevent successful attacks.
...
### [31] [Koopman Invariants as Drivers of Emergent Time-Series Clustering in Joint-Embedding Predictive Architectures](https://arxiv.org/abs/2511.09783)
...
### [32] [CaReTS: A Multi-Task Framework Unifying Classification and Regression for Time Series Forecasting](https://arxiv.org/abs/2511.09789)
...
### [33] [Beyond Monotonicity: Revisiting Factorization Principles in Multi-Agent Q-Learning](https://arxiv.org/abs/2511.09792)
...
### [34] [Constrained Best Arm Identification with Tests for Feasibility](https://arxiv.org/abs/2511.09808)
...
### [35] [On the Convergence of Overparameterized Problems: Inherent Properties of the Compositional Structure of Neural Networks](https://arxiv.org/abs/2511.09810)
...
### [36] [Learning Intersections of Halfspaces under Factorizable Distribution](https://arxiv.org/abs/2511.09832)
...
### [37] [ACT as Human: Multimodal Large Language Model Data Annotation with Critical Thinking](https://arxiv.org/abs/2511.09833)
...
### [38] [Steering Pretrained Drafters during Speculative Decoding](https://arxiv.org/abs/2511.09844)
...
### [39] [ConSurv: Multimodal Continual Learning for Survival Analysis](https://arxiv.org/abs/2511.09853)
...
### [40] [Unlearning Imperative: Securing Trustworthy and Responsible LLMs through Engineered Forgetting](https://arxiv.org/abs/2511.09855)
*James Jin Kang,Dang Bui,Thanh Pham,Huo-Chong Ling*

Main category: cs.LG

TL;DR: 对LLMs的机器无学习进行系统综述，发现缺乏可验证且鲁棒的永久忘记机制，需更高效的无再训练技术及治理框架。


<details>
  <summary>Details</summary>
Motivation: 在敏感领域部署LLMs时，必须保证私有信息能够永久忘记；现有方法成本高、碎片化、难以验证，且易受对抗恢复攻击。

Method: 系统性评估近年关于LLMs无学习的研究，考察忘记判定、对抗攻击鲁棒性，以及在透明度受限条件下的信任机制；同时评估包括差分隐私、同态加密、联邦学习、短暂记忆等技术及审计与法规等制度性方案。

Result: 显示出持续进展，但真正可验证的忘记仍未实现；需要更高效的无再训练技术、对抗恢复的更强防护，以及更完善的治理结构。

Conclusion: 通过整合技术与组织视角，提出在保/privacy与公信力前提下实现“可忘记”的AI系统的路径。
...
### [41] [Uncertainty-Guided Checkpoint Selection for Reinforcement Finetuning of Large Language Models](https://arxiv.org/abs/2511.09864)
...
### [42] [A General Anchor-Based Framework for Scalable Fair Clustering](https://arxiv.org/abs/2511.09889)
*Shengfei Wei,Suyuan Liu,Jun Wang,Ke Liang,Miaomiao Li,Lei Luo*

Main category: cs.LG

TL;DR: AFCF 提供一个线性时间可扩展的公平聚类框架，通过锚点采样和锚点图传播实现任意公平聚类算法的加速，同时在理论上保证对整个数据集的公平性与在大规模数据集上的有效性。


<details>
  <summary>Details</summary>
Motivation: 现有的公平聚类在计算复杂度上通常是二次或更高，难以处理大规模数据集，迫切需要一种通用、可插拔且具线性时间复杂性的解决方案以实现高效且公平的聚类。

Method: 提出基于锚点的公平聚类框架：首先用一种新的公平采样策略选取一组小而具代表性的锚点；对该小的锚点集合应用现成的公平聚类算法。核心是锚点图构建：通过一个优化问题在锚点之间传播标签，同时保持公平性，通过精心设计的分组-标签联合约束，理论证明锚点聚类的公平性等价于全数据集的公平性。该优化使用基于 ADMM 的算法高效求解。大量在大规模基准数据上的实验显示，该框架能将现有方法的计算时间降低数量级，同时保持良好的聚类性能和公平性保证。

Result: 在大规模基准数据集上，AFCF 显著加速了最先进的公平聚类方法，时间成本降低到原方法的数量级，同时维持强劲的聚类性能与公平性保证。

Conclusion: AFCF 提供了一种通用且可扩展的公平聚类解决方案，具备线性时间的可扩展性与理论上的公平性保真性，便于在大规模数据集上实际部署。
...
### [43] [Simulator and Experience Enhanced Diffusion Model for Comprehensive ECG Generation](https://arxiv.org/abs/2511.09895)
...
TL;DR: 提出 M^2V-UGAD，一种在不完备图上的无监督图异常检测框架，通过双路径编码器分别重建缺失属性和图结构，并在联合潜在空间中区分法线/异常，同时通过在正常区域边界外采样潜在编码来生成硬负样本；在七个公开数据集上对比，持续优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 解决现实场景中图的属性和边缺失，以及因修复引入的偏差和跨视图干扰对异常检测的影响。

Method: 双路径编码器：独立重建缺失的属性和结构；将两路径在一个联合潜在空间中融合并正则化，使正常样本聚成内在紧凑的流形，异常处于外壳；为了缓解修复偏差，在正常区域边界外采样潜在编码并解码成真实的节点特征和子图，形成硬负样本以强化判别边界。

Result: 在七个公开基准数据集上，方法在不同缺失率下均优于现有无监督GAD方法。

Conclusion: M^2V-UGAD 有效应对不完整图中的缺失值对无监督GAD的影响，减小修复偏差，提升检测鲁棒性。
...
TL;DR: EEGAgent是一个通用框架，利用大语言模型协调多种工具，对EEG进行预处理、特征提取、事件检测等，以实现多任务、可扩展的脑电分析，并可生成报告。


<details>
  <summary>Details</summary>
Motivation: 现有EEG分析多为针对单一任务的模型，在多任务、连续推理场景中缺乏通用性和可扩展性，需要一个可解释、可控的分析框架来提升临床和研究应用。

Method: 构建基于工具箱的EEG分析系统，将LLMs用于调度与规划各子工具（如EEG预处理、特征提取、事件检测等），实现EEG信息感知、时空探索、事件检测、与用户交互以及报告生成等功能。

Result: 在公开数据集上进行评估，表明EEGAgent能够实现灵活且可解释的EEG分析，具有潜在的真实世界临床应用价值。

Conclusion: EEGAgent提供一个通用、模块化的框架，能够处理多任务EEG分析并促进其在临床和研究中的落地。
...
TL;DR: 提出动态阈值以改进漂移检测；证明基于数据分段的最佳阈值组合策略优于任何单一固定阈值；并提出 Dynamic Threshold Determination（DTD）算法，在多数据集上验证效果，显著提升现有漂移检测器性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法将阈值视为固定超参数，易导致性能在不同数据集和时间上波动，动态阈值可更好地维持模型性能。

Method: 给出理论证明：通过把各个数据分段的最佳阈值组合起来，所得到的动态策略在性能上可超过任何单一阈值；在此基础提出 DTD 算法，增加一个比较阶段以决定阈值调整；将其嵌入现有漂移检测框架。

Result: 在大量合成与真实数据集（包括图像和表格数据）上进行广泛实验，结果显示所提出方法显著提升了最先进漂移检测器的性能。

Conclusion: 动态阈值具有理论与实践双重优势，能够在不同数据段中自适应地选择阈值从而提升检测效果；该框架具有较高的普适性。
...
TL;DR: Open-world 多模态学习的鲁棒性是核心挑战，当前实验条件下的表现与实际部署之间存在显著差距。本文对这一问题进行概念性分析与框架性探讨。


<details>
  <summary>Details</summary>
Motivation: 在不确定、动态变化的开放世界中，环境组成的不可预测性、模态输入的缺失，以及分布关系的偏差，会严重影响系统的可靠性。尽管人类能够灵活适应，现有AI在处理多模态信号时鲁棒性不足，制约实际应用。

Method: 对开放世界中的多模态学习鲁棒性问题进行系统性分析，明确核心瓶颈，提出未来研究方向以缩小实验室性能与实际部署之间的差距。

Result: 文章未给出具体实证结果；提供了一套概念性分析框架、问题清单和未来研究方向，用以指导后续的实证研究。

Conclusion: 需要发展自适应模态融合、容错缺失模态、对分布偏移与虚假相关具有鲁棒性的机制，以提升开放世界中多模态系统的可靠性。
...
### [53] [Interaction as Interference: A Quantum-Inspired Aggregation Approach](https://arxiv.org/abs/2511.10018)
*Pilsung Kang*

Main category: cs.LG

TL;DR: 提出一种量子启发的干涉聚合框架 IKC，通过对相位的直接机制控制实现协同/拮抗；引入 Coherent Gain 和 Interference Information 指标，在轻量线性振幅模型中，干涉跨项与 Δ_INT 等价；在 XOR 和真实表格数据上进行实验，结果显示固定参数时相干聚合提升预测不确定性的度量，且在数据集上表现有竞争力但有时落后于容量更高的基线。


<details>
  <summary>Details</summary>
Motivation: 现有交互建模多以工程化的产物项或自洽的模型模式呈现，缺乏对交互产生机制的可控性与解释性。通过量子概率的“干涉”思想，尝试在统计模型中引入一个可控的相位机制，以直接调节协同与拮抗的强度，从而获得对交互效应的更清晰控制和诊断能力。

Method: 提出 Interference Kernel Classifier (IKC)，在一个最小线性振幅模型中通过 coherent 与 incoherent 聚合的差异引入干涉跨项，使其与常见的潜在结果交互对比 Δ_INT 对应；开发 Coherent Gain（对数似然增益）与 Interference Information（KL 散度增量）两项诊断；通过相位扫描确认身份；在 XOR、Adult、Bank Marketing 数据上进行对比实验，评估在配对、预算匹配条件下的性能与校准。

Result: 在高交互的合成任务 XOR 上，IKC 在配对、预算匹配条件下优于强基线；在真实表格数据 Adult 与 Bank Marketing 上总体具有竞争力，但通常落后于容量更高的强基线；将学习参数固定时，将聚合从 incoherent 切换为 coherent，负对数似然、Brier score 与期望校准误差普遍改善，且 Coherent Gain 为正。

Conclusion: 提出的干涉聚合框架为交互控制提供一种机制级的解释与操作手段，通过相干聚合提升概率估计的信息质量与校准性；结果在合成任务上显示明显优势，在真实数据上具有竞争力但仍受限于模型容量。未来可探讨更大尺度的数据、不同相位的学习策略以及对干涉效应的更细粒度调控。
...
### [54] [GraphSB: Boosting Imbalanced Node Classification on Graphs through Structural Balance](https://arxiv.org/abs/2511.10022)
...
TL;DR: 该工作讨论在大规模MoE语言模型中由于内存限制而将非活跃专家临时移到CPU，并用预取隐藏PCIe传输延迟，但预取失败会带来显著的延迟或准确性下降；核心任务是确保在预取失败时仍能保持高推理速度与模型准确性。


<details>
  <summary>Details</summary>
Motivation: MoE模型参数规模远超单机GPU内存，必须通过外部存储及预取来实现可扩展性；现有策略在预取失败时存在速度-准确性权衡，亟需解决这一瓶颈。

Method: 分析现有系统的内存管理、预取策略及失败情景，指出在预取失败时的两种主流解决方案（按需获取或丢弃专家）的局限性，以及对保持速度与准确性的挑战。

Result: 摘要未给出具体实验结果或新方法，仅界定了问题域、挑战与现有方法的局限性。

Conclusion: 在预取失败场景下同时维持高推理速度和模型准确性是核心挑战，后续工作需要提出鲁棒的策略来缓解PCIe瓶颈带来的额外延迟并保护模型性能。
...
TL;DR: 提出FAQNAS，在HQNN中将FLOPs纳入多目标NAS，找到在保持精度的同时显著降低FLOPs的架构，表明在NISQ时代FLOPs也是关键的设计指标。


<details>
  <summary>Details</summary>
Motivation: 在NISQ时代，量子电路并非天然以FLOPs衡量，但在用经典模拟器训练HQNN时，FLOPs直接决定运行时和可扩展性，因此需要一种将FLOPs纳入优化目标的NAS框架。

Method: 提出FAQNAS，将HQNN设计视为多目标优化问题，显式把FLOPs引入目标函数，搜索包含量子电路与经典层的架构空间，利用Pareto前沿寻找在精度与FLOPs之间的权衡解，实验在五个数据集上展示。

Result: 在MNIST、Digits、Wine、Breast Cancer、Iris数据集上，量子FLOPs对精度提升的贡献大于经典FLOPs，经典FLOPs相对固定。Pareto前沿显示，在FLOPs不变或降低的情况下，仍可达到有竞争力的准确率，相比不考虑FLOPs的基线。

Conclusion: FLOPs感知是NISQ时代HQNN设计的实用准则，也是未来HQNN系统的可扩展原则。
...
### [59] [eXIAA: eXplainable Injections for Adversarial Attack](https://arxiv.org/abs/2511.10088)
...
### [60] [T2IBias: Uncovering Societal Bias Encoded in the Latent Space of Text-to-Image Generative Models](https://arxiv.org/abs/2511.10089)
*Abu Sufian,Cosimo Distante,Marco Leo,Hanan Salam*

Main category: cs.LG

TL;DR: 五大开源文本到图像模型的潜在潜在空间编码并放大社会性别/种族偏见，尤其在职业生成中表现明显；不同模型呈现不同偏好模式；并给出缓解策略与风险意识建议。


<details>
  <summary>Details</summary>
Motivation: 解决生成式AI在现实应用中的伦理风险，避免性别与种族刻板印象被放大对组织伦理造成冲击。

Method: 对五个主流开源T2I模型进行横向实验，使用十个中性职业提示生成100张图像/职业，总计5000张；由来自不同种族和性别的评估者进行人工评估；分析输出的偏见模式并识别模型特定行为。

Result: 所有模型都编码并放大显著的社会偏见：护理等职业偏向女性化；高地位职业如CEO、政治家、医生、律师等偏向男性和白人；模型间存在特征模式：QWEN-Image偏向东亚输出、Kandinsky偏向白人、SDXL输出范围较广但仍具偏见。研究可用于帮助AI项目管理者在模型选择和定制提示时实现更公平的生成。

Conclusion: 提出可操作的偏见缓解策略，强调风险并为负责任的生成式AI系统提供落地建议。
...
### [61] [How does My Model Fail? Automatic Identification and Interpretation of Physical Plausibility Failure Modes with Matryoshka Transcoders](https://arxiv.org/abs/2511.10094)
...
### [62] [RI-Loss: A Learnable Residual-Informed Loss for Time Series Forecasting](https://arxiv.org/abs/2511.10130)
*Jieting Wang,Xiaolei Shang,Feijiang Li,Furong Peng*

Main category: cs.LG

TL;DR: 提出基于HSIC的RI-Loss，针对时间序列预测中的残差结构进行噪声感知的端到端损失，提升对噪声与时序关系的建模能力，并给出非渐近的HSIC界与双样本复杂度分析，实验证明对多模型多数据集有效。


<details>
  <summary>Details</summary>
Motivation: 现有MSE在时间序列预测中有两大不足：1) 逐点误差忽视了时序关系，2) 未考虑数据中的固有噪声。需要一种能够显式建模残差与噪声结构的目标函数，以提升鲁棒性和泛化性。

Method: 提出残差信息化损失RI-Loss，通过最大化或度量残差序列与一个随机时间序列之间的依赖性（基于Hilbert-Schmidt独立性准则HSIC）来建模噪声结构。给出关于RI-Loss的第一条非渐近HSIC界，含有显式的双样本复杂度项，并通过Bernstein型不等式和Rademacher复杂度分析证明收敛速率。通过将RI-Loss与常见的时间序列预测模型结合，进行端到端训练。

Result: 在八个真实世界基准和五种主流预测模型上进行广泛实验，RI-Loss带来预测性能的提升，验证了噪声感知表示的有效性和鲁棒性。代码将公开以提高复现性。

Conclusion: RI-Loss提供了对HSIC框架的理论保证与实证收益，能在不同模型和数据上提升时间序列预测的鲁棒性和准确性，未来工作可进一步扩展到其他序列建模任务并优化计算效率。
...
### [63] [EPO: Diverse and Realistic Protein Ensemble Generation via Energy Preference Optimization](https://arxiv.org/abs/2511.10165)
*Yuancheng Sun,Yuxuan Ren,Zhaoming Chen,Xu Han,Kang Liu,Qiwei Ye*

Main category: cs.LG

TL;DR: 提出 Energy Preference Optimization (EPO) 在线 refinement 将预训练蛋白构象生成器转变为能量感知采样器，无需额外 MD 轨迹。通过 SDE 采样探索构象空间，结合基于列表偏好优化的能量排序机制，并引入能量轨迹概率的可行上界来近似难以求解的连续时间生成模型中长采样轨迹的概率。结果在 Tetrapeptides、ATLAS、Fast-Folding 基准上实现多样且物理现实的构象集合，达到九项评价指标的最新水平。结论强调只利用能量偏好信号即可引导生成模型生成热力学一致的构象集合，提供替代漫长 MD 的可行方案。


<details>
  <summary>Details</summary>
Motivation: 准确探索蛋白构象谱对理解功能至关重要，但传统 MD 计算成本高且易受能垒困扰，需要更高效的生成模型驱动的采样方法来获得热力学一致的构象 ensembles。

Method: 在一个预训练的蛋白构象生成器上进行在线 refinement（EPO），成为一个能量感知的采样器；采用随机微分方程(SDE)采样来遍历构象景观；引入基于列表偏好优化的能量排序机制实现能量排名；为近似连续时间生成模型中长采样轨迹的不可解概率，提供一个实用上界以便与现有生成器对接。

Result: 在 Tetrapeptides、ATLAS、Fast-Folding 基准上，EPO 能生成多样而物理现实的构象集合，达到九项评估指标的最新最佳水平。表现出能量仅偏好信号即可有效引导生成模型达到热力学一致性。

Conclusion: 能量偏好信号足以高效引导生成模型产生热力学一致的蛋白构象集合，提供了替代漫长 MD 的可行路径，并拓展了 learned potentials 在结构生物学与药物发现中的应用。
...
### [64] [Improved Offline Reinforcement Learning via Quantum Metric Encoding](https://arxiv.org/abs/2511.10187)
*Outongyi Lv,Yewei Yuan,Nana Liu*

Main category: cs.LG

TL;DR: 引入量子度量编码器（QME）来应对有限样本下的离线强化学习问题。通过将状态嵌入到量子电路启发的紧凑表征中，改变状态空间的几何结构，从而显著提升在有限样本条件下的离线RL性能（SAC和IQL在三组各100样本数据集上的最大奖励平均提升分别为116.2%和117.6%）。对经典数据，QME为可经典仿真、可训练的单位嵌入；对量子数据可直接在量子硬件上实现并训练。还发现QME嵌入具有较低的Δ-双曲性，暗示几何的改变量是性能提升的关键。


<details>
  <summary>Details</summary>
Motivation: 多数现实世界的强化学习问题都面临极少量的训练样本，离线RL在此场景下往往表现不佳。需要通过更有效的状态表示或几何结构来提高样本低时的学习效率。引入量子启发的编码结构（QME）作为一个潜在的高效特征表示模块。

Method: 在三组限定为100个样本的数据集上，分别使用SAC和IQL两种离线RL算法，比较在原始状态-原始奖励与经过QME嵌入后再解码奖励的离线RL性能。对经典数据，QME为可经典仿真的可训练单位嵌入；对量子数据，直接在量子硬件上实现并训练。还对嵌入后的状态空间进行Δ-双曲性分析，探讨几何结构的变化与学习效果的关系。

Result: 与原始状态/奖励相比，QME嵌入的离线RL在最大奖励上有显著提升：SAC平均提升约116.2%，IQL平均提升约117.6%。Δ-双曲性分析显示QME嵌入的状态空间具有较低的Δ-双曲性，与训练效果提升相关。

Conclusion: QME通过改变状态空间的几何结构，尤其是在有限样本条件下，显著提升离线RL的性能。Δ-双曲性的降低为该效果提供了几何层面的解释，提示未来在离线RL中关注嵌入的几何属性作为提升手段。
...
### [65] [Towards Leveraging Sequential Structure in Animal Vocalizations](https://arxiv.org/abs/2511.10190)
*Eklavya Sarkar,Mathew Magimai. -Doss*

Main category: cs.LG

TL;DR: 通过对自监督语音模型 HuBERT 表示进行向量量化和 Gumbel-softmax 向量量化得到的离散声学令牌序列，捕获动物发声的时间顺序信息，并在四个数据集上通过令牌序列的 Levenshtein 距离和 kNN 分类显示其对叫声类型和发声者具有区分能力，展现了将序列信息用于动物声学分析的潜力。


<details>
  <summary>Details</summary>
Motivation: 传统生物声学研究往往将帧级特征在时间维度上取平均，忽略了发声单元的顺序结构。本文尝试用离散化的声学令牌序列来编码时间信息，评估其在识别叫声类型和发声者方面的有效性。

Method: 从自监督 HuBERT 表征中通过向量量化及 Gumbel-softmax 量化得到离散令牌；对令牌序列进行配对距离分析以区分叫声类型和发声者；并使用 kNN 分类，采用 Levenshtein 距离作为序列相似度。

Result: 不同数据集上，离散令牌序列能够区分叫声类型和发声者；基于 Levenshtein 距离的 kNN 分类呈现出较为可观的性能，证明令牌序列是潜在的替代特征表示，能有效利用动物发声的时序信息。

Conclusion: 离散化的令牌序列为捕捉动物发声的时序结构提供了有效途径，具有在跨数据集的识别任务中用于类型与说话者识别的应用潜力，未来可进一步优化量化策略和序列比较方法。
...
### [66] [Beyond MSE: Ordinal Cross-Entropy for Probabilistic Time Series Forecasting](https://arxiv.org/abs/2511.10200)
*Jieting Wang,Huimei Shi,Feijiang Li,Xiaolei Shang*

Main category: cs.LG

TL;DR: 提出 OCE-TS：一个基于有序分类的时间序列预测新框架，用有序交叉熵替代 MSE，输出概率分布以实现不确定性估计与对离群点的鲁棒性。通过将观测值离散化为有序区间并预测每个时间步的分布，利用累积分布上的交叉熵保持有序关系。


<details>
  <summary>Details</summary>
Motivation: 传统深度学习预测多用 MSE，无法给出不确定性且对离群值鲁棒性差；需要保留预测的有序性并能量化不确定性。

Method: 将观测值离散化为有序区间并用一个简单线性模型预测每个时间步的区间分布；以参数分布估计的概率作为监督信号，建立 OCE 损失，在预测和真实的累积分布之间计算交叉熵，以显式保持预测值的有序关系。同时通过影响度函数理论分析，证明 CE 相较于 MSE 在稳定性和对离群点的鲁棒性上更优。

Result: 在七个公开数据集上与 Autoformer、DLinear、iTransformer、TimeXer、TimeBridge 等基线模型比较，使用 MSE/MAE 指标，OCE-TS 显示出对所有数据集的优越表现。

Conclusion: OCE-TS 提供了一个可观测的概率输出与不确定性估计的时间序列预测方案，且从理论和经验上表明 CE 损失在该框架中比 MSE 更稳定、对离群点更鲁棒；代码将对外发布。
...
### [67] [Fractional neural attention for efficient multiscale sequence processing](https://arxiv.org/abs/2511.10208)
...
### [68] [Out-of-Context Misinformation Detection via Variational Domain-Invariant Learning with Test-Time Training](https://arxiv.org/abs/2511.10213)
*Xi Yang,Han Zhang,Zhijian Lin,Yibiao Hu,Hong Han*

Main category: cs.LG

TL;DR: 提出 VDT：通过域不变变分对齐、域一致性重建和测试时训练实现 OOC 偏误检测的域自适应方法，在 NewsCLIPpings 数据集上优于大多数基线。


<details>
  <summary>Details</summary>
Motivation: OOC 信息在新闻报道中低成本制造，跨域分布差异导致现有方法在新领域表现差，需域自适应与测试时自适应来提升泛化。

Method: 引入域不变变分对齐（Domain-Invariant Variational Align）模块，联合编码源/目标域数据以学习域不变的可分离分布；域一致性约束模块用于重建潜在分布以保持语义完整；测试阶段采用测试时训练策略和置信-方差筛选模块动态更新 VAE 编码器和分类器以适应目标域分布。

Result: 在基准数据集 NewsCLIPpings 上在大多数域自适应设定下，方法优于最先进的基线。

Conclusion: 通过对齐分布并在测试时自适应，VDT 显著提升 OOC 识别的跨域泛化能力。
...
### [69] [FedCure: Mitigating Participation Bias in Semi-Asynchronous Federated Learning with Non-IID Data](https://arxiv.org/abs/2511.10227)
...
### [70] [Lost in Serialization: Invariance and Generalization of LLM Graph Reasoners](https://arxiv.org/abs/2511.10234)
*Daniel Herbst,Lea Karbeska,Divyanshu Kumar,Akanksha Ahuja,Fatemeh Gholamzadeh Nasrabadi,Fabrizio Frasca*

Main category: cs.LG

TL;DR: LLMs用于图推理的鲁棒性受限于图序列化的对称性；通过将序列化分解为节点标注、边编码和语法，系统评估不同因素的鲁棒性，并提出光谱任务以评估泛化。


<details>
  <summary>Details</summary>
Motivation: 理解并提升基于LLMs的图家族推理在不同图表示下的一致性和鲁棒性，特别是在节点重标、边顺序和格式变换下的输出稳定性。

Method: 提出图序列化的分解模型（节点标注、边编码、语法），构建全面的基准评测并分析不同因素对鲁棒性的影响；在此基础上设计一组新的光谱任务来测试已微调模型的泛化能力；比较不同模型规模及微调对鲁棒性和泛化的影响。

Result: 更大、未微调的模型在鲁棒性上更强；微调降低对节点重标的敏感性，但可能增加对结构和格式变换的敏感性，且对未见任务的性能提升并不稳定。

Conclusion: 在图表示对称性下，基于LLMs的图推理仍面临鲁棒性挑战；模型规模提升有帮助，但微调并非泛化的万用钥匙，需针对性设计与评估；提出的分解视角与光谱任务有助于系统性评估与改进。
...
### [71] [Heuristic Transformer: Belief Augmented In-Context Reinforcement Learning](https://arxiv.org/abs/2511.10251)
...
### [72] [Unitho: A Unified Multi-Task Framework for Computational Lithography](https://arxiv.org/abs/2511.10255)
...
### [73] [Torch-Uncertainty: A Deep Learning Framework for Uncertainty Quantification](https://arxiv.org/abs/2511.10282)
*Adrien Lafage,Olivier Laurent,Firas Gabetni,Gianni Franchi*

Main category: cs.LG

TL;DR: 提出 Torch-Uncertainty 框架，基于 PyTorch/Lightning，统一评估与集成不确定性量化（UQ）方法，覆盖分类、分割与回归任务，并给出基准实验结果。


<details>
  <summary>Details</summary>
Motivation: 当前缺乏一个无缝工作流来评估和集成多种 UQ 方法，导致对预测不确定性的可靠性评估难以在现实应用中落地；需要一个统一、可扩展的工具来提升 DNN 不确定性估计的可信度。

Method: 开发 Torch-Uncertainty 库，基于 PyTorch 和 Lightning，提供统一的训练和评估流程，集成多种 UQ 方法与评估指标，并在分类、分割、回归任务上进行系统化实验基准。

Result: 给出对多种 UQ 方法在不同任务上的全面基准实验，展示库的可用性与扩展性；工具代码在 GitHub 上公开（链接）。

Conclusion: 该框架为评估与整合 UQ 方法提供了统一的工作流，帮助研究者与实践者在实际场景中更可靠地量化与比较不确定性。
...
### [75] [PITE: Multi-Prototype Alignment for Individual Treatment Effect Estimation](https://arxiv.org/abs/2511.10320)
...
### [77] [Gradient Flow Equations for Deep Linear Neural Networks: A Survey from a Network Perspective](https://arxiv.org/abs/2511.10362)
...
### [78] [Product distribution learning with imperfect advice](https://arxiv.org/abs/2511.10366)
*Arnab Bhattacharyya,Davin Choo,Philips George John,Themis Gouleakis*

Main category: cs.LG

TL;DR: 若给定一个与真实分布 P 的均值向量接近的先验 Q（p 与 q 的 L1 距离满足条件），则存在一个高效算法能以 tilde-O(d^{1-η}/ε^2) 的样本复杂度学习 P 到 TV 距离 ε；前提是 ||p-q||_1 < ε d^{0.5-Ω(η)}，且算法对该距离下界未知。


<details>
  <summary>Details</summary>
Motivation: 研究在有先验信息的情况下降低学习乘积分布的样本复杂度。

Method: 提出自适应利用 Q 的信息的学习算法，在不需要事先知道距离界的情况下实现改进。

Result: 在给定条件下，样本复杂度为 tilde-O(d^{1-η}/ε^2)。

Conclusion: 先验均值信息可显著降低对高维布尔乘积分布的学习难度，且算法能自适应利用该信息而无需预知距离界。
...
### [79] [Enhancing Kernel Power K-means: Scalable and Robust Clustering with Random Fourier Features and Possibilistic Method](https://arxiv.org/abs/2511.10392)
*Yixi Chen,Weixuan Liang,Tianrui Liu,Jun-Jie Huang,Ao Li,Xueling Zhu,Xinwang Liu*

Main category: cs.LG

TL;DR: 提出了 RFF-KPKM 和 IP-RFF-MKPKM，通过随机傅里叶特征近似核映射实现对核功率 k-均值的高效、可扩展聚类，并给出理论保证与在大规模数据上的优越性能。


<details>
  <summary>Details</summary>
Motivation: 解决 KPKM 的两大瓶颈：一是完整核矩阵带来的计算开销，二是缺乏真实的中心-样本分配学习造成的鲁棒性不足，以及支持多核学习的需求。

Method: 使用随机傅里叶特征（RFF）近似核映射来构造低维特征，从而避免构建全核矩阵。提出 RFF-KPKM；进一步提出 IP-RFF-MKPKM，将 possibilistic 与 fuzzy membership 相结合，以提升鲁棒性并支持多核学习。给出对该组合的严格理论保证： excess risk bound 为 O(√(k^3/n))，在带有 membership 的强一致性，以及利用维度为 poly(ε^{-1} log k) 的 RFF 获得 (1+ε) 的相对误差界。实验还显示在大规模数据集上优于现有方法。

Result: 理论方面给出无偏差的风险界、强一致性及相对误差界；方法上实现了高效的近似与鲁棒的多核学习能力；实验部分在大规模数据集上展现出更高的效率与聚类准确性。

Conclusion: 通过将 RFF 与核功率 k-均值结合，RFF-KPKM 实现了可扩展且鲁棒的聚类，同时 IP-RFF-MKPKM 提供对多核学习的改进与更稳健的分配，适用于大规模聚类任务。
...
### [80] [AgentEvolver: Towards Efficient Self-Evolving Agent System](https://arxiv.org/abs/2511.10395)
...
### [81] [Unlocking Dynamic Inter-Client Spatial Dependencies: A Federated Spatio-Temporal Graph Learning Method for Traffic Flow Forecasting](https://arxiv.org/abs/2511.10434)
...
### [82] [Neuronal Fluctuations: Learning Rates vs Participating Neurons](https://arxiv.org/abs/2511.10435)
...
TL;DR: 将不确定性校准与基于扰动的解释联系起来，证明解释期望在经历解释专用扰动时会出现错配，从而削弱解释质量；并提出 ReCalX，通过重新校准模型来提升解释质量而不改变预测，在多模型/数据集上实现了对扰动相关错 calibration 的显著降低以及对全局/局部重要特征识别的改进。


<details>
  <summary>Details</summary>
Motivation: 研究不确定性校准与基于扰动的解释之间的关系，揭示解释可靠性在面对解释专用扰动时的弱点，并提出改进解释质量的需求。

Method: 理论证明：不校准会直接削弱全局和局部解释质量；提出 ReCalX 作为一种重新校准方法，目标是在不改变预测的前提下提升解释一致性和鲁棒性；通过跨多种模型和数据集的实验评价其在减少扰动相关错 calibration、提升解释鲁棒性及全球输入特征重要性识别方面的效果。

Result: ReCalX 在不同模型和数据集上稳定地降低了扰动专用的错 calibration，并提升了解释的鲁棒性及对全局重要特征的识别能力。

Conclusion: 通过重新校准来提升解释质量的策略可在不改变原有预测的前提下提升扰动下的解释可靠性，且具有较强的泛化能力。
...
### [84] [Intrinsic Dimensionality as a Model-Free Measure of Class Imbalance](https://arxiv.org/abs/2511.10475)
...
### [85] [Panda: Test-Time Adaptation with Negative Data Augmentation](https://arxiv.org/abs/2511.10481)
...
### [86] [Weak Relation Enforcement for Kinematic-Informed Long-Term Stock Prediction with Artificial Neural Networks](https://arxiv.org/abs/2511.10494)
*Stanislav Selitskiy*
...
### [87] [Holonorm](https://arxiv.org/abs/2511.10504)
...
### [88] [Maximizing Efficiency of Dataset Compression for Machine Learning Potentials With Information Theory](https://arxiv.org/abs/2511.10561)
...
### [89] [Oya: Deep Learning for Accurate Global Precipitation Estimation](https://arxiv.org/abs/2511.10562)
...
### [90] [Impact of Layer Norm on Memorization and Generalization in Transformers](https://arxiv.org/abs/2511.10566)
...
### [91] [Belief Net: A Filter-Based Framework for Learning Hidden Markov Models from Observations](https://arxiv.org/abs/2511.10571)
...
### [92] [Towards Emotionally Intelligent and Responsible Reinforcement Learning](https://arxiv.org/abs/2511.10573)
...
### [93] [Semi-Unified Sparse Dictionary Learning with Learnable Top-K LISTA and FISTA Encoders](https://arxiv.org/abs/2511.10575)
...
### [94] [Tight Robustness Certification through the Convex Hull of $\ell_0$ Attacks](https://arxiv.org/abs/2511.10576)
...
### [95] [Pretrained Joint Predictions for Scalable Batch Bayesian Optimization of Molecular Designs](https://arxiv.org/abs/2511.10590)
